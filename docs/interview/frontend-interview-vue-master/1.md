...
父组件可以向子组件传递`style`和`class`，它们会**合并**到子组件的**根元素**中
...
如果父组件传递了一些属性到子组件，但子组件并没有声明这些属性，则它们称之为`attribute`，这些属性会**直接附着在子组件的根元素上**

> - 不包括`style`和`class`，它们会被特殊处理
>
> - attrs 和 props 区别 [javascript - What is the difference between properties and attributes in HTML? - Stack Overflow](https://stackoverflow.com/questions/6003819/what-is-the-difference-between-properties-and-attributes-in-html#answer-6004028) 
...
子组件可以通过`inheritAttrs: false`配置，**禁止**将`attribute`**附着**在子组件的根元素上，但不影响通过`$attrs`获取

### `natvie`修饰符

在注册事件时，父组件可以使用`native`修饰符，将事件注册到**子组件的根元素**上
...
子组件可以通过`$listeners`获取父组件传递过来的所有事件处理函数，能获**取返回结果**。

经常配合 `v-on` 使用

结合`attribute`模拟一个普通的 `<input>` 元素例子
