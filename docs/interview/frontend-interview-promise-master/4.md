...
这个例子中 *while* 循环为什么不会被卡死？

> 评论回答：浏览器不会被卡死的原因是，死循环是在一个微队列中，代码上的体现就是他处于一个async异步函数中，
  ...
  <script>
    ...
    function getElement(cssSelector) {
      ...
      const proxy = new Proxy(dom, {
        get(target, key) {
          if (!key.startsWith("await")) return Reflect.get(target, key);
          const event = key.replace("await", "").toLowerCase();
