...
<img src="https://qwq9527.gitee.io/resource/imgs/image-20200417161950450.png" alt="img"  align="left"/>
...
![image-20200422172837190](https://qwq9527.gitee.io/resource/imgs/image-20200422172837190.png)
...
- alg：signature部分使用的签名算法，通常可以取两个值
  ...
  - RS256：一种非对称加密算法，使用私钥签名，公钥验证
    ...
    > [RS256和HS256的区别，他们与SHA-256的联系 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/505799575)
...
- iss：(Issuer) 发行该jwt的是谁，可以写公司名字，也可以写服务名称
- sub：(Subject) 该jwt是用于干嘛的
- iat：(Issued At) 该jwt的发放时间，通常写当前时间的时间戳
- exp：(Expiration Time) 该jwt的到期时间，通常写时间戳
- aud： (Audience) 该 jwt 是发放给哪个终端的，可以是终端类型，也可以是用户名称，随意一点
- nbf：(Not Before) 一个时间点，在该时间点到达之前，这个令牌是不可用的
