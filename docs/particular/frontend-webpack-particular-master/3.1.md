...
> 这种方案在手机端的React Native中大行其道（因为手机端不存在css样式表，最终形成ui对象）
...
module.exports={
    primaryColor:"#f40"// 解决变量重复的问题，可以定义一个模块，预设原始值，后续只需要改动这里即可
}
var styles={
    color:require("...").primaryColor, 
    backgroundColor:"xxx",
    fontSize:xxx
}
// 写成函数
...
var commmonStyle=require("./commonStyle");
var styles={
    ...commonStyle,
    ...别的样式 // 还可以覆盖公共样式，非常灵活
}    
// 混合：上面也是混合的例子
// 提取：从一个样式对象提取某些属性组成一个对象
function take(styles,...args){}
take(commonStyle,"color");--> {color:"#f40"}    
```
...
这种方案虽然可以利用js语言解决重复样式值的问题，但由于太过激进(*选择器都不存在了*)，很多习惯写css的开发者编写起来并不是很适应

```js
// 模块：color.js
export const danger = "#f40";
