...
上图表示：把*css*源码交给*postcss*,*postcss*首先对源码进行分析，*Parser*这一步，相当于把*css*分析成一棵抽象的语法树。

分析完后，将结果也就是抽象语法树依次交给插件，每个插件更改抽象的语法树，将结果交给下一个插件，等所有插件更改完之后，*postcss*将抽象的语法树转成*css*字符串。
...
- `map:{inline:false}`表示源码地图不在文件行内以base64位方式存在。而是单独生成一个*.map*文件。
...
安装`npm i stylelint` `npm i stylelint-config-standard`
...
```json
{
    "extends": "stylelint-config-standard",
    "rules": {
        "indentation": null // indentation表示规则名称
    }
}
```

设置为`null`可以**禁用该规则**，或者设置为4，表示一个缩进有4个空格。具体的设置需要参见stylelint文档：[https://stylelint.io/](https://stylelint.io/)
...
[postcss/postcss: Transforming styles with JS plugins (github.com)](https://github.com/postcss/postcss#css-in-js)
